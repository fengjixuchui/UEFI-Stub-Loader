...
rem output binary, which is called "STUBLOAD.EFI"
rem

@echo on
"%GCC_FOLDER_NAME%\bin\gcc.exe" -nostdlib -Wl,--warn-common -Wl,--no-undefined -Wl,-dll -s -shared -Wl,--subsystem,10 -e efi_main -Wl,-Map=output.map -o "STUBLOAD.EFI" @"objects.list"
@echo off
...
"%GCC_FOLDER_NAME%\bin\size.exe" "STUBLOAD.EFI"
echo.
